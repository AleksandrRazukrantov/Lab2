...
> Программа получает на вход 2 числа a0(первый элемент последовательности)  и n(количество элементов последовательности). Нужно, из условия, определить принцип последовательности и вывести последовательность, состоящую из n элементов, где нулевым членом будет а0. 
...
3. **Сосдание последовательности:**
    Используем цикл for(пробегаем от 0 до n(не включительно), таким образом получим нужную длину последовательность): в теле цикла переприсваиваем значение а0 на а0*2, далее проверяем если а0 > 18, то вычетаем из а0 18. Выводим а0 на экран.
...
public class Main {
    ...
    public static void main(String[] args) {
       ...
       int N = in.nextInt();
       System.out.println(((int)Math.pow(3,N) - 1)/2);
    }
}
...
2. **Поиск:**  
   Сначала, проверяем уменьшаеться прирост насиления или нет, для того чтобы прирост населения уменьшался нужно: ((P0 * g / 100) + m) < 0, если верно, выводим "Население микрарайона постоянно уменьшаеться", иначе создаем переменную n, для подсчета лет, используем цикл while(пока Р0 < Сар): в теле цикла переприсваеваем значение Р0 на (Р0 + (P0 * g / 100) + m) и прибавляем 1 к n.
